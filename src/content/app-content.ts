...
const appContentRu: AppPageContent = {
  ...
  storeLinks: [{ href: officialAppStoreUrl, label: "App Store", meta: "Загрузить для iPhone и iPad" }, { href: officialGooglePlayUrl, label: "Google Play", meta: "Загрузить для Android" }],
...
const appContentUz: AppPageContent = {
  ...
  storeLinks: [{ href: officialAppStoreUrl, label: "App Store", meta: "iPhone va iPad uchun yuklash" }, { href: officialGooglePlayUrl, label: "Google Play", meta: "Android uchun yuklash" }],
...
export function getAppPageContent(locale: SupportedSiteLocale) {
  const localized = getGeneratedPageContent(
    locale,
    "app",
    appContentByLocale[locale] ?? appContentEn,
  );

  // Store URLs are operational data, not translatable copy. Generated locale
  // payloads may localize labels and descriptions, but must never replace them.
  return {
    ...localized,
    storeLinks: localized.storeLinks.map((store, index) => ({
      ...store,
      href:
        index === 0
          ? officialAppStoreUrl
