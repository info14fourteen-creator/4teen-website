...
export type WhitepaperVersionSlug = "v1-0" | "v1-1" | "v1-2" | "v1-3";
...
const whitepaperContentEn: WhitepaperPageContent = {
  metadata: {
    title: "Whitepaper",
    description:
      "Read the current 4TEEN whitepaper and the preserved full historical versions v1.2, v1.1, and v1.0 without losing the original document text.",
  },
  hero: {
    eyebrow: "Whitepaper",
    title: "Current version plus preserved historical versions.",
    lead:
      "This route now treats the whitepaper as a versioned reading surface. The current v1.3 stays on /whitepaper, and the earlier v1.2, v1.1, and v1.0 remain available as separate pages with their full original text preserved.",
  },
  versionPicker: {
    eyebrow: "Version Picker",
    title: "Switch between the current document and the full archive.",
    body:
...
Positioning:
liquidity automation is part of the operating infrastructure, not part of the rule-making layer.
It triggers the routine. The contract determines whether the routine is valid.
...
In practical terms, the system continues after registration.
Registration opens the path. Cabinet and worker logic keep that path alive.
...
The ambassador system is built for operational safety, not only for immediate settlement.
Verified purchases may be accepted and stored first, then settled on-chain later when resource conditions allow.

Positioning:
this system is best understood as four connected layers working together —
frontend referral capture and registration, ambassador cabinet, backend allocation worker, and on-chain controller settlement.
It is not just a referral helper, not just a dashboard, and not just a worker.
