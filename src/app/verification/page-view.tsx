...
export function getVerificationPageMetadata(
  locale: SupportedSiteLocale = defaultSiteLocale,
): Metadata {
  const metadata = getVerificationPageContent(locale).metadata;
  return buildPageMetadata({ ...metadata, locale, pathname: "/verification" });
}

export const metadata: Metadata = getVerificationPageMetadata();

export async function VerificationPageView({
  ...
  const snapshot =
    await getServerSiteSnapshot<LiveVerificationSnapshot>("verification");
          ...
          <article className="ft-card ft-card--strong ft-placeholder-hero">
            <div className="ft-stack ft-stack--lg">
              <div className="ft-cluster ft-cluster--sm">
                ...
                <SiteSnapshotRefresh snapshotKeys={["verification"]} />
              </div>
